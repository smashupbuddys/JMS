-- Drop all existing payment validation functions and triggers
...
DROP FUNCTION IF EXISTS validate_quotation_payment_details() CASCADE;

-- Create function to validate payment array with explicit argument name
...
-- Create enhanced payment details validation function with explicit argument name
...
BEGIN
  ...
  BEGIN
    v_total_amount := (p_details->>'total_amount')::numeric;
    v_paid_amount := (p_details->>'paid_amount')::numeric;
    v_pending_amount := (p_details->>'pending_amount')::numeric;
...
-- Create trigger function with explicit argument name
