/*
  # Update Sales Tracking Triggers

  1. Changes
    - Fix trigger conditions to avoid referencing OLD values in INSERT triggers
    - Split triggers into separate INSERT and UPDATE triggers
    - Add separate handling for new and updated sales

  2. Features
    - Track completed sales for both new and updated records
    - Update daily sales metrics
    - Update manufacturer performance metrics
    - Handle both retail and wholesale sales
*/
...
DROP TRIGGER IF EXISTS completed_sale_insert_trigger ON quotations;
CREATE TRIGGER completed_sale_insert_trigger
...
DROP TRIGGER IF EXISTS completed_sale_update_trigger ON quotations;
CREATE TRIGGER completed_sale_update_trigger
  AFTER UPDATE ON quotations
  FOR EACH ROW
  WHEN (NEW.status = 'accepted' AND NEW.payment_details->>'payment_status' = 'completed' AND
        (OLD.status != 'accepted' OR OLD.payment_details->>'payment_status' != 'completed'))
  EXECUTE FUNCTION update_daily_sales();

-- Create separate triggers for manufacturer performance
DROP TRIGGER IF EXISTS manufacturer_performance_insert_trigger ON quotations;
CREATE TRIGGER manufacturer_performance_insert_trigger
  AFTER INSERT ON quotations
  FOR EACH ROW
  WHEN (NEW.status = 'accepted' AND NEW.payment_details->>'payment_status' = 'completed')
  EXECUTE FUNCTION update_manufacturer_performance();

DROP TRIGGER IF EXISTS manufacturer_performance_update_trigger ON quotations;
CREATE TRIGGER manufacturer_performance_update_trigger
  AFTER UPDATE ON quotations
  FOR EACH ROW
  WHEN (NEW.status = 'accepted' AND NEW.payment_details->>'payment_status' = 'completed' AND
        (OLD.status != 'accepted' OR OLD.payment_details->>'payment_status' != 'completed'))
